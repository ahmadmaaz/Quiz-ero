...
function Quiz(props) {
    ...
    useEffect(()=>{
      ...
      if(props.url){
        axios.get(props.url+"/score").then((response)=>{
    ...
    const postQuiz = (answers_,time)=>{
      setTime(time)
      setDone(true)
      axios.post(props.url,{answers:answers_,time:time,id:id,key:localStorage.getItem("Key_QA")}).then((response)=>{
            ...
            <div className="w-full  lg:w-2/4  xl:w-2/4 md:w-2/4  ">
                {id ? 
                  <div>
                    { done ? 
                      <div>
                        <ScoreDisplay time={time} score={score} url={props.url}/> 
                        <hr></hr>

...
function QueBox(props) {
    ...
    const I =useRef(0);
    let ques=props.ques;
    //classes style for click event
    const [classBox1,setClassBox1]= useState("font-mono text-black bg-sky-0  p-2 mb-2 rounded text-xs md:text-base w-8/12 lg:w-2/4 text-center border-2  border-sky-700  hover:text-white hover:bg-sky-700  ");
    const [classBox2,setClassBox2]= useState("font-mono text-black bg-sky-0  p-2 mb-2 rounded w-3/4 text-center text-xs md:text-base border-2   border-sky-700  lg:hover:text-white hover:bg-sky-0 lg:hover:bg-sky-700 ring-4 ring-sky-700 border-0");
  ...
  return (
    <div className='pb-28'> 
        <form onSubmit={nextQue} className="text-center mt-8">
            ...
            <button className={clickbtn1 ?  classBox2 :classBox1} type="button" onClick={newAnswer} id="choice1" >{choice1}</button> <br></br>
            <button className={clickbtn2 ?  classBox2 :classBox1} type="button" onClick={newAnswer} id="choice2" >{choice2}</button> <br></br>
            <button className={clickbtn3 ?  classBox2 :classBox1} type="button" onClick={newAnswer} id="choice3" >{choice3}</button><br></br>
        ...
        <div className='flex '>  <div className='font-mono font-bold bg-yellow-600 hover:bg-yellow-800 rounded px-4 py-2 m-4 w-max z-0 text-white'> {minutes}:{seconds<10 ? "0" + seconds: seconds }</div>      <div className='font-mono w-max m-auto text-base font-bold lg:text-3xl italic antialiased  md:text-xl '> Question {I.current}</div> </div>

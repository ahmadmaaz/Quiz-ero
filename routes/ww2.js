...
router.get("/",(req,res)=>{
    ...
    db.query("Select * from quizww2", (err,result)=>{
...
router.post("/",(req,res)=>{
    ...
    db.query((`Select * from ww2_score where ipAddress='${ip}'  && keyUser=${key}`),(err,result_)=>{
        if (err) {throw err;res.sendStatus(500)};
        
        if(result_.length===0){
            db.query(`insert into ww2_score (ipAddress,keyUser,name,score,timeSpent,timesDone,City) VALUES("${ip}",${key},"${name}",${result},"${req.body.time}",1,"${Place}" )`);
        }
        else if(req.body.time <result_[0].timeSpent && result_[0].score===result){
            db.query(`Update ww2_score set timeSpent="${req.body.time}", timesDone=${result_[0].timesDone +1} where ipAddress="${ip}" && keyUser=${key}`);
        }else if (result >result_[0].score){
            db.query(`Update ww2_score set score=${result},timeSpent="${req.body.time}", timesDone=${result_[0].timesDone +1} where ipAddress="${ip}" && keyUser=${key}`);
        } else{
            db.query(`Update ww2_score set timesDone=${result_[0].timesDone +1} where ipAddress="${ip}" && keyUser=${key}`)
        } 
    })
...
router.get("/score",(req,res)=>{
    ...
    db.query(("Select name,score,timeSpent,timesDone,City from ww2_score order by score DESC ,timeSpent ASC,timesDone ASC limit 10"),(err,result)=>{
        if(err) throw err
        res.send(result)
    })
})
